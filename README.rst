...
- An INSEE API key (generated on `portail-api.insee.fr <https://portail-api.insee.fr>`_)
...
.. list-table::
   ...
   * - ``siren``
     - ``Char(9)``
     - SIREN — manually entered or updated via wizard
   * - ``siret``
     - ``Char(14)``
     - Registered office SIRET — manually entered or updated via wizard
...
.. list-table::
   ...
   * - Field
     - Description
   * - ``sirene_denomination``
     - Denomination returned by the API (with sigle if present)
   * - ``sirene_siren``
     - SIREN returned by the API
   * - ``sirene_siret_siege``
     - Registered office SIRET returned by the API
   * - ``sirene_naf``
     - NAF code (formatted, e.g. ``62.02A``)
   * - ``sirene_naf_activity``
     - NAF activity label
   * - ``sirene_date_creation``
     - Legal creation date
   * - ``sirene_legal_form_code``
     - Catégorie juridique code (4 digits, e.g. ``5499``)
   * - ``sirene_legal_form``
     - Catégorie juridique label (e.g. ``Société à responsabilité limitée (SARL)``)
   * - ``sirene_workforce``
     - Workforce range label with reference year (e.g. ``20 à 49 salariés (2022)``)
   * - ``sirene_categorie_entreprise``
     - Company category: ``PME``, ``ETI`` or ``GE``
   * - ``sirene_etat_administratif``
     - Administrative status: ``A`` (Active) or ``C`` (Ceased)
   * - ``sirene_date_cessation``
     - Cessation date (when status is ``C``)
   * - ``sirene_industry_id``
     - Odoo industry mapped from the NAF NACE section
   * - ``sirene_sync_state``
     - Sync state: ``unchecked``, ``ok``, ``pending``, ``ignored``, ``error``
   * - ``sirene_last_check_date``
     - Timestamp of the last API call
   * - ``sirene_last_error``
     - Error message from the last failed API call
   * - ``sirene_etablissement_ids``
     - Related establishments (model ``sirene.etablissement``)
